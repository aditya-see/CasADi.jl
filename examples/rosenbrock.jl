...
f = x^2 + 100*z^2
g = z + (1-x)^2 - y
nlp = Dict("x"=>casadi.vertcat(x,y,z), "f"=>f, "g"=>g)
...
# Print solution
println()
println("Optimal cost:", res["f"])
println("Primal solution:", res["x"])
println("Dual solution (simple bounds):", res["lam_x"])
println("Dual solution (nonlinear bounds):", res["lam_g"])
